...
const CreateTamplateTargetCluster = observer(({ open, onClose, onComplete }) => {
  const {
    targetClusters,
    unselectedClusters,
    setTargetClusters,
  ...
  const { loadCluster } = clusterStore;
              ...
              {selectedClusters?.map((targetCluster, index) => (
                <Droppable droppableId={index.toString()}>
                  {(provided, snapshot) => (
                    ...
                    >
                      <DeleteButton onClick={() => deleteLeveled(index)}>x</DeleteButton>
...
export default CreateTamplateTargetCluster;
